...
def create_operator_mcp(client: DispatchAPIClient, config: MCPConfig) -> FastMCP:
    ...
    async def list_namespaces(request: ListNamespacesRequest) -> ListNamespacesResponse:
        ...
        Args:
            request: ListNamespacesRequest (no parameters)
    ...
    async def create_agent(request: CreateAgentRequest) -> CreateAgentResponse:
        ...
        Returns:
            CreateAgentResponse with success status, agent_directory path, and message
        """
        ns = _get_namespace(request.namespace)
        ...
        dispatch_yaml_content = f"""agent_name: {request.agent_name}
namespace: {ns}
"""
    ...
    async def get_deploy_status(
        request: GetDeployStatusRequest,
        ...
        Args:
            request: GetDeployStatusRequest with job_id and optional namespace

        Returns:
            GetDeployStatusResponse with status, stages, logs, and error details
        """
        ns = _get_namespace(request.namespace)
    ...
    async def uninstall_agent(request: UninstallAgentRequest) -> UninstallAgentResponse:
        ...
        Args:
            request: UninstallAgentRequest with agent_name and namespace

        Returns:
            UninstallAgentResponse with deletion status and agent_id
        """
        ns = _get_namespace(request.namespace)
    ...
    async def get_agent_logs(request: GetAgentLogsRequest) -> GetAgentLogsResponse:
        ...
        Args:
            request: GetAgentLogsRequest with agent_name, namespace, version, and limit

        Returns:
            GetAgentLogsResponse with events array and next_token
        """
        ns = _get_namespace(request.namespace)
    ...
    async def get_topic_schema(
        ...
        Args:
            request: GetTopicSchemaRequest with topic and namespace

        Returns:
            GetTopicSchemaResponse with topic, canonical_schema, and handlers
        """
        ns = _get_namespace(request.namespace)
    ...
    async def publish_event(request: PublishEventRequest) -> PublishEventResponse:
        ...
        Args:
            request: PublishEventRequest with topic, payload, and namespace

        Returns:
            PublishEventResponse with message and event_uid
        """
        ns = _get_namespace(request.namespace)
    ...
    async def get_agent_functions(
        ...
        Args:
            request: GetAgentFunctionsRequest with agent_name and namespace

        Returns:
            GetAgentFunctionsResponse with list of functions and their schemas
        """
        ns = _get_namespace(request.namespace)
    ...
    async def invoke_function(
        ...
        Args:
            request: InvokeFunctionRequest with agent_name, function_name, payload
            ctx: MCP context for logging

        Returns:
            InvokeFunctionResponse with status and result (if waited)
        """
        ns = _get_namespace(request.namespace)
        ...
        max_polls = int(
            (request.timeout_seconds or 300) / request.poll_interval_seconds
        )
        ...
        return InvokeFunctionResponse(
            ...
            status="TIMEOUT",
            result=None,
            error=f"Timed out after {request.timeout_seconds} seconds",
            waited=True,
        )

    @mcp.tool()
    async def get_invocation_status(
        request: GetInvocationStatusRequest,
        ...
        Args:
            request: GetInvocationStatusRequest with invocation_id

        Returns:
            GetInvocationStatusResponse with status, result, and timestamps
        """
        ns = _get_namespace(request.namespace)
    ...
    async def start_local_agent_dev(
        ...
        Returns:
            StartLocalAgentDevResponse with agent_name, status, and message
        """
        # Convert to absolute path for consistency
        abs_agent_dir = os.path.abspath(request.agent_directory)
        router_port = request.router_port
        ...
        try:
            ...
            await ctx.debug("Router is running")
        except Exception:
        ...
        if os.path.exists(pid_file):
            await ctx.debug("Found PID file for existing agent process...")
            try:
                with open(pid_file) as f:
                ...
                try:
                    ...
                    await ctx.debug(f"Process {old_pid} already stopped")
                except PermissionError:
                    await ctx.warning(f"No permission to kill process {old_pid}")

                # Clean up stale PID file
                os.remove(pid_file)
            except (OSError, ValueError) as e:
                await ctx.debug(f"Error reading PID file: {e}")

        reload_msg = " with hot-reload" if request.enable_reload else ""
        await ctx.info(f"Starting dev mode for agent '{agent_name}'{reload_msg}")
            ...
            try:
                if process.stdout:
                    async for line in process.stdout:
                        line_str = line.decode().strip()
                        if line_str:
                            await ctx.debug(f"[{agent_name}] {line_str}")
                if process.stderr:
                    async for line in process.stderr:
                        line_str = line.decode().strip()
                        if line_str:
                ...
                await ctx.error(f"Error monitoring {agent_name}: {e}")
    ...
    @mcp.tool()
    async def stop_local_router(
        request: StopRouterRequest,
        ctx: Context[ServerSession, None],
    ) -> StopRouterResponse:
        ...
        if process.returncode != 0:
            ...
            if (
                ...
                await ctx.info("Router is already stopped")

                # Still clean up any agent dev processes using PID files
                await ctx.info("Cleaning up agent dev processes...")
                cleaned = await cleanup_all_agent_processes()
                if cleaned > 0:
                    await ctx.info(f"Stopped {cleaned} agent process(es)")
            ...
            await ctx.error(f"Failed to stop router: {error_msg}")
            raise RuntimeError(f"Failed to stop router: {error_msg}")

        await ctx.info("Router stopped successfully")
    ...
    async def list_local_routers(
        ...
        Args:
            request: ListRoutersRequest (no parameters)
            ctx: MCP context for logging

        Returns:
            ListRoutersResponse with list of tracked routers
        """
        from dispatch_cli.commands.router import get_tracked_routers

        await ctx.debug("Listing tracked routers")
        ...
        return ListRoutersResponse(routers=routers, total=len(routers))

    @mcp.tool()
    async def send_local_test_event(
        request: SendTestEventRequest,
        ctx: Context[ServerSession, None],
    ) -> SendTestEventResponse:
        """Send a test event through the local router using dispatch router test.

        Args:
            request: SendTestEventRequest with topic and payload
            ctx: MCP context for logging

        Returns:
            SendTestEventResponse with status and message
        """
        await ctx.info(f"Sending test event to topic '{request.topic}'")

        # Call the router API directly
        local_router_url = f"{LOCAL_ROUTER_URL}:{request.router_port}"
        event_data = {"payload": request.payload, "sender_id": "mcp-test"}

        try:
            timeout = aiohttp.ClientTimeout(total=60.0 * 60 * 24)  # 24 hours
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(
                    f"{local_router_url}/api/unstable/events/{request.topic}",
                    json=event_data,
                ) as response:
                    response.raise_for_status()
            ...
            await ctx.info(f"Event routed to {len(routed_to)} agent(s)")
            ...
            await ctx.error(f"Failed to send event: {error_msg}")
    ...
    async def invoke_local_function(
        request: InvokeLocalFunctionRequest,
        ctx: Context[ServerSession, None],
    ) -> InvokeLocalFunctionResponse:
        ...
        Args:
            request: InvokeLocalFunctionRequest with agent_directory, function_name, payload
            ctx: MCP context for logging

        Returns:
            InvokeLocalFunctionResponse with status and result
        """
        ...
        await ctx.info(
            f"Invoking function '{request.function_name}' on agent '{agent_name}'"
        )
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                await ctx.debug(f"Invocation started: {invocation_id}")

                # Step 2: Poll for completion
                poll_interval = 0.2  # 200ms between polls
                max_polls = int(request.timeout_seconds / poll_interval)
                ...
                for _ in range(max_polls):
                    ...
                    if status == "completed":
                        ...
                        await ctx.info(
                            f"Function '{request.function_name}' completed successfully"
                        )
                        return InvokeLocalFunctionResponse(
                            status="success",
                            result=result,
                    ...
                    if status == "error":
                        ...
                        await ctx.error(f"Function failed: {error}")
                        return InvokeLocalFunctionResponse(
                            status="error",
                            result=None,
                            error=error,
                ...
                return InvokeLocalFunctionResponse(
                    status="error",
                    result=None,
                    error=f"Timeout after {request.timeout_seconds}s waiting for function to complete",
                    agent_name=agent_name,
                )

        except aiohttp.ClientError as e:
            error_msg = f"HTTP error: {e}"
            await ctx.error(f"Failed to invoke function: {error_msg}")
            return InvokeLocalFunctionResponse(
                status="error",
                result=None,
                error=error_msg,
    ...
    async def read_local_agent_logs(
        request: ReadLocalAgentLogsRequest,
        ctx: Context[ServerSession, None],
    ) -> ReadLocalAgentLogsResponse:
        ...
        Args:
            request: ReadLocalAgentLogsRequest with agent_directory and optional lines count
            ctx: MCP context for logging

        Returns:
            ReadLocalAgentLogsResponse with agent name, logs, and total lines
        """
        # Convert to absolute path for consistency
        abs_agent_dir = os.path.abspath(request.agent_directory)
        agent_name = os.path.basename(abs_agent_dir)

        await ctx.debug(f"Reading logs for agent '{agent_name}'...")

        # Use helper function to read logs
        try:
            logs = read_agent_log_file(abs_agent_dir, lines=request.lines)
            await ctx.info(f"Retrieved {len(logs)} log lines for agent '{agent_name}'")
    ...
    async def create_schedule(
        ...
        Args:
            request: CreateScheduleRequest with agent_name, function_name, cron_expression,
                     and optional timezone, payload, description, timeout_seconds

        Returns:
            CreateScheduleResponse with schedule_id and confirmation message
        """
        return await client.create_schedule(request)
    ...
    async def list_schedules(
        ...
        Returns:
            ListSchedulesResponse with list of schedules and total count
        """
        return await client.list_schedules(request)
    ...
    async def get_schedule(
        ...
        Returns:
            GetScheduleResponse with full schedule details
        """
        return await client.get_schedule(request)
    ...
    async def update_schedule(
        ...
        """
        return await client.update_schedule(request)
    ...
    async def delete_schedule(
        ...
        """
        return await client.delete_schedule(request)
